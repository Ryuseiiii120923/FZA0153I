...
<body>
    ...
    <table class="defect-table">
        ...
        @php
            ...
            $totalNgSum       = $totals['summary']['total_ng'] ?? 0;
            ...
            $largeDefectTotals = collect($totals['large_defects'])->keyBy('defect');
            $smallDefectTotals = collect($totals['small_defects'])
                ->keyBy(fn($item) => $item['large_defect'] . '||' . $item['small_defect']);
            $reworkTotals      = collect($totals['reworks'])->keyBy('rework_type');
        @endphp
        <tr style="font-weight:bold; background-color:#f0f0f0;">
            ...
            @foreach($items as $defect)
            @php
                $key           = $largeCategory . '||' . $defect['small_category'];
                $smallCatTotal = $smallDefectTotals->get($key)['total_qty'] ?? 0;
            @endphp
