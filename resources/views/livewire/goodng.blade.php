<div id="OuterPanel" class="outer-panel">
    <div class="flex flex-col sm:flex-row  justify-center gap-4 mt-4 items-center">
        <div class="flex-col w-11/12 sm:w-1/3 mx-5 sm:mx-2 ">
            ...
            <input type="number" id="excss" inputmode="numeric"
                autocomplete="off"
                autocorrect="off"
                autocapitalize="off" class="mt-1 block w-full border border-black rounded-md px-2 py-1 @if($locked) opacity-20 cursor-not-allowed @endif" @if($locked) readonly @endif
    ...
    autocomplete="off" 
    autocorrect="off" 
    autocapitalize="off" class="mt-1 block w-full border border-black rounded-md px-2 py-1  @if ($locklack) bg-gray-500 @endif  @if($locked) opacity-20 cursor-not-allowed @endif" @if($locked) readonly @endif
    ...
    autocapitalize="off" class="mt-1 block w-full border border-black rounded-md px-2 py-1 @if($locked) opacity-20 cursor-not-allowed @endif" @if($locked) readonly @endif
        ...
        <div class="flex-col w-11/12 sm:w-1/3 mx-5 sm:mx-2 ">
            ...
            <input type="number" id="good"  class="mt-1 block w-full border border-black rounded-md px-2 py-1 @if($locked) opacity-20 cursor-not-allowed @endif" @if($locked) readonly @endif
        ...
        <div class="flex-col w-11/12 sm:w-1/3 mx-5 sm:mx-2 ">
            ...
            <input type="number" id="ng" class="mt-1 block w-full border border-black rounded-md px-2 py-1 @if($locked) opacity-20 cursor-not-allowed @endif" @if($locked) readonly @endif
