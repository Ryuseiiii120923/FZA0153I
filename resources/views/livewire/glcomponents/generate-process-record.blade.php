<div class="w-full relative"
    ...
    {{-- Global loading overlay (search + pagination) --}}
    <div
        wire:loading
        wire:target="search, updatingSearch, perPage, gotoPage, nextPage, previousPage, setPage"
        class="absolute inset-0 z-50 flex items-center justify-center rounded-xl bg-black/60 backdrop-blur-[10px]">
        <div class="flex flex-col items-center gap-3">
            ...
            <span class="text-white text-sm font-medium tracking-wide">Loading...</span>
        </div>
    </div>
    ...
    @if($records === null)
        <div class="flex flex-col items-center justify-center py-24 gap-3 text-gray-400">
            ...
            <p class="text-sm">Enter a PPF number in the search box to get started.</p>
        </div>
    ...
    @else
        ...
        <div class="mt-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 text-sm text-white">
            <p>
                Showing
                <span class="font-medium">{{ $records->firstItem() }}</span>–<span class="font-medium">{{ $records->lastItem() }}</span>
                of
                <span class="font-medium">{{ $records->total() }}</span>
                PPF {{ Str::plural('number', $records->total()) }}
                @if($search)
                    matching <span class="font-medium">"{{ $search }}"</span>
                @endif
            </p>
